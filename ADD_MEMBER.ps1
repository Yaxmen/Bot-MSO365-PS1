...
if(!$Tickets){
    ...
    foreach($ticket in $tickets){
	    ...
	    $DistributionGroup = Get-DistributionGroup -Identity $ticket.IdentificadorDaLista -ErrorAction Stop
            ...
            $CloseNote = "Conforme solicitado, o(s) membros(os) informado no chamado foi inserido ao grupo solicitado"
            $ItemDeCausa = "Outlook"
            $Causa = "Insert Member"
