...
05-05-23 Inclusão de categoria SEMI-RESTRITO na oferta do Service Now e tratamento de erro do Add-AzureGroupMember
24-05-23 Inclusão de novo prefixo
12-07-23 Inclusão de novos prefixos
...
##           Log "$UPN;ERRO: $($disp_MCOEV) $SKU - insuficientes para configurar usuario"
	    ...
	    try { Add-AzureADGroupMember -ObjectId $GUIDLIC -RefObjectId $OnlineUser.Identity } catch { }
        ...
        while ($NOK) {
            ...
            if ($($resp.FeatureTypes) -ccontains "PhoneSystem") {
                ...
                try { Set-MsolUserLicense -UserPrincipalName $UPN -AddLicenses "$SKU" } catch { }
                $SLEEP = 20
                $SLEEPT += $SLEEP
                Log "$UPN;ATENCAO: aguardando aplicar licença do phoneSystem aguardar + $SLEEP seg"
                if ($SLEEPT -gt 600) {
